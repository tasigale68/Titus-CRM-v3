...
function getWeeklyReports(req, res) {
  try {
    ...
    airtable.fetchAllFromTable("Weekly Stakeholder Reports").then(function(records) {
      ...
      res.status(500).json({ error: "Failed to fetch weekly reports: " + e.message });
    });
  } catch (err) {
    console.error("Weekly reports GET unexpected error:", err.message);
    res.status(500).json({ error: "Failed to fetch weekly reports: " + err.message });
  }
}
...
function postWeeklyReport(req, res) {
  try {
    ...
    if (!env.airtable.apiKey || !env.airtable.baseId) return res.status(400).json({ error: "Airtable not configured" });
  ...
  if (clientName) {
    ...
    console.error("Weekly report POST unexpected error:", err.message);
    res.status(500).json({ error: "Weekly report save failed: " + err.message });
  }
}
...
function generateStakeholderData(req, res, b) {
  ...
  if (clientNames.length === 0) return res.status(400).json({ error: 'No clients specified' });
  ...
  Promise.all([companyP, progressP, incidentsP, clientsP, contactsP, mediaP]).then(function(results) {
    ...
    if (anthropicKey && participants.length > 0) {
      ...
      Promise.all(aiPromises).then(function() {
        ...
        console.error('Stakeholder AI analysis error:', aiErr.message);
        // Still return data even if AI failed
        res.json({ company: company, participants: participants });
      });
    } else {
      res.json({ company: company, participants: participants });
    ...
    console.error('Stakeholder generate unexpected error:', outerErr.message);
    res.status(500).json({ error: 'Failed to generate stakeholder data: ' + outerErr.message });
  }
}
