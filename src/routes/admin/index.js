...
// Secured: superadmin only + one-time token
...
router.post('/migrate-to-supabase', function (req, res) {
  if (!isAdminRole(req.user)) return res.status(403).json({ error: 'Superadmin only' });
  ...
  console.log('[MIGRATION] Started by ' + req.user.email + ' at ' + new Date().toISOString());
  ...
  // Generic mapper — stores all Airtable fields as JSONB
